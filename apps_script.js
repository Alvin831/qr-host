...
function doGet(e) {
  try {
    ...
    if (path === "presence/qr/generate") return generateQR(e.parameter);
...
function generateQR(body) {
  ...
  const now = new Date();
  const exp = new Date(now.getTime() + 5 * 60 * 1000); // 5 menit
...
function checkin(body) {
  const requiredError = validateRequired(
    ["user_id", "device_id", "course_id", "session_id", "qr_token"],
    body
  );
  if (requiredError) return outputError(requiredError);
